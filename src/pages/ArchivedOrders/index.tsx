...
function ArchivedOrders() {
      ...
      (async () => {
        try {
          ...
          const { data } = await axios.get(
            `${baseUrl}/order/admin/get-archived-orders-dates?page=${offset}&limit=6&searchString=${search}&timeZone=${targetTimeZone}`,
  ...
  const downloadAllArchivedOrdersHandler = async () => {
    try {
      setIsDownloadPdfLoading(true);
      const { data } = await axios.get(
        `${baseUrl}/generate-pdf/archived-orders`,
        TokenConfig(tabIdentifier)
      );
  ...
  const downloadIndividualArchivedOrdersHandler = async (date: string, date1: string) => {
    try {
      setDownloadId(date);
      setIsPdfGenerationInProgress(true);
      const { data } = await axios.post(
        `${baseUrl}/generate-pdf/get-archived-orders-by-date`,
  ...
  if (isError && !isLoading) {
    return (
      <EmptyOrErrorState text="Oops,an error occurred while fetching arhived orders 😕" />
    );
  }
    ...
    <Container sx={{ mt: 0.5 }}>
      ...
      >
        ...
        <Button
          ...
          onClick={downloadAllArchivedOrdersHandler}
        >
          Download all as ZIP
        </Button>
      </Stack>
